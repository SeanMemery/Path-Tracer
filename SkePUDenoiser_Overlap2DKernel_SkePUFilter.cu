...
__global__ void SkePUDenoiser_Overlap2DKernel_SkePUFilter_conv_cuda_2D_kernel(FilterVals* skepu_output, skepu::PRNG::Placeholder,struct GPUInf *skepu_input, 
	...
	skepu::Edge skepu_edge, struct GPUInf skepu_pad
)
{
  extern __shared__ struct GPUInf sdata_skepu_skel_0[];
	...
	if (skepu_x < skepu_out_cols && skepu_y < skepu_out_rows)
	{
		...
		auto skepu_res = skepu_userfunction_skepu_skel_0convol_SkePUFilter::CU({(int)skepu_overlap_y, (int)skepu_overlap_x, skepu_sharedCols, &sdata_skepu_skel_0[(threadIdx.y + skepu_overlap_y) * skepu_sharedCols + (threadIdx.x + skepu_overlap_x)]});
