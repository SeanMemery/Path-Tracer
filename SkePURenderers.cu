    ...
    static ReturnStruct SkepuRenderFunc(skepu::Index2D ind, RandomSeeds seeds, Constants constants) {
        ...
        */
        ...
        if (constants.getDenoiserInf == 1) {
            ...
            double tempFB[3] = {rayPositions[1][0] - rayPositions[0][0], 
                                rayPositions[1][1] - rayPositions[0][1], 
                                rayPositions[1][2] - rayPositions[0][2]};
            double fbn = sqrt(tempFB[0]*tempFB[0] + tempFB[1]*tempFB[1] + tempFB[2]*tempFB[2]);
            ret.firstBounce[0] = tempFB[0]/fbn; 
            ret.firstBounce[1] = tempFB[1]/fbn;
            ret.firstBounce[2] = tempFB[2]/fbn;
            double tempDepthRay[3] = {rayPositions[0][0] - constants.camPos[0], 
                                    rayPositions[0][1] - constants.camPos[1], 
                                    rayPositions[0][2] - constants.camPos[2]};
            ret.depth = sqrt(tempDepthRay[0]*tempDepthRay[0] + tempDepthRay[1]*tempDepthRay[1] + tempDepthRay[2]*tempDepthRay[2]);
...
struct skepu_userfunction_skepu_skel_0renderFunc_SkepuRenderFunc
{
...
#include "skepu_skel_0_SkePURenderers_MapKernel_SkepuRenderFunc.cu"
void Renderers::SkePURender() {

        // Configure SkePU
        auto spec = skepu::BackendSpec{skepu::Backend::typeFromString(skepuBackend)};
        spec.activateBackend();
        skepu::backend::Map<1, skepu_userfunction_skepu_skel_0renderFunc_SkepuRenderFunc, decltype(&skepu_skel_0_SkePURenderers_MapKernel_SkepuRenderFunc), void> renderFunc(skepu_skel_0_SkePURenderers_MapKernel_SkepuRenderFunc);
